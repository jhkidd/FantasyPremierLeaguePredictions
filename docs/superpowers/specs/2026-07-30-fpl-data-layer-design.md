...
**Four tiers of component availability.** Not every component reaches equally far back. This table was revised on 2026-07-31 after directly probing the vaastav archive (`docs/superpowers/plans/2026-07-31-fpl-data-layer-phases-4-6-plan.md`) — the pre-2019 FPL API exposed a materially richer stat set than the 2019/20–2024/25 API did, and it was restored only partly in 2025/26. The original version of this table, based on the 2025/26 API alone, understated availability.

| Tier | Components | Depth |
|---|---|---|
| Deep | minutes, goals, assists, clean sheets, goals conceded, saves, cards, penalties, own goals | ~10 seasons |
| Shallow (defensive contribution) | clearances, blocks, interceptions, tackles, recoveries | **2016/17–2018/19 and 2025/26 — four seasons**, not one. Verified populated (non-zero among 60+ minute players): CBI 6,609/6,560/6,462 for 2016/17–2018/19 respectively. Absent 2019/20–2024/25. |
| Shallow (BPS inputs) | passes attempted/completed, key passes, big chances created/missed, open-play crosses, dribbles, tackled, fouls, offside, penalties conceded, errors leading to goal/attempt | **2016/17–2018/19 only.** Still missing in those seasons: shots on target, saves in the box, saves from a big chance, goalline clearances, fouls won. |
| **Not derivable** | **bonus, exactly** | **no season** — see below |

Bonus is the sharp problem, though a narrower one than first thought. FPL publishes the BPS *total* but not all of its inputs in any season. For 2016/17–2018/19, most of the BPS table **is** available (previous row), which converts bonus from "model blind from proxies" into "model against a measured residual" — see the BPS reproducibility report (§ below, produced in phase 6) for how much of BPS those seasons actually reconstruct and how often the gap changes the top-3 bonus ordering within a match. For 2019/20–2024/25, only the CBI/tackle/recovery-adjacent proxies are available and the residual is materially larger. Historical `bps` totals also carry a known bias under 2026/27 rules (CBI now scores 1 BPS per 3 rather than per 2; being tackled no longer costs −1; a penalty save is 7 not 8).
...
**Consequence: `facts/player_fixture` carries a per-row availability mask.** Implemented as one boolean per *component group* (defensive, BPS-input, expected-stats, starts) rather than one per column — in practice only four independent presence patterns occur across the seven schema eras, so grouping loses no information at a tenth the width. Without it a model reads ten seasons of missing tackle data as ten seasons of zero tackles, which is not a subtle error — it is a systematic one concentrated exactly on the defenders the shallow tier exists to evaluate.

Backfilling further than the FPL API itself already provides was investigated and rejected. Every third-party source with Opta-compatible definitions and per-match depth (Sofascore, FotMob, WhoScored) prohibits redistribution from a public repository, and Football DataCo database rights apply on top of the site terms. The legally clean alternatives cover one Premier League season each (Wyscout 2017/18 under CC BY 4.0; StatsBomb 2015/16), have no 2025/26 overlap to validate against, and Wyscout's "recovery" is a possession-level event rather than Opta's per-player one — so it cannot reproduce the `R` term that the midfielder and forward threshold of 12 depends on. Mislabelled rows at exactly the threshold would be worse than no backfill. **We use the four seasons the FPL API itself provides (2016/17–2018/19, 2025/26) and accept no defensive-contribution training data for 2019/20–2024/25.**

**Open risk carried forward, not resolved here:** there is no season of overlap between 2018/19 and 2025/26, so nothing in this archive can prove Opta's CBI/tackle/recovery definitions held constant across the seven-year gap in which the stat was dropped from the public API. This is a materially weaker objection than the one raised against third-party vendors above — these are FPL's own fields from FPL's own supplier, not a different vendor's ontology — but it is not zero. Facts are masked by *era*, not merely by presence, so training with and without the pre-2019 seasons can be compared directly once models exist.
...
FPL player IDs, Understat player IDs and football-data team names do not agree, and FPL's own `id`/`element` is reassigned between seasons.

**Revised 2026-07-31 after probing the archive directly.** Cross-*season* FPL identity turns out not to need fuzzy matching at all: every season's `players_raw.csv` carries a `code` field that is stable across seasons even when `id` is not. Verified: of 270 players present in both 2016/17 and 2019/20, 269 had a different `id` but all 270 kept the same `code`, with zero collisions — the only disagreements were 9 players whose name had been re-spelled between seasons (e.g. accents restored), not different people sharing a code. `id_dict.csv`, the file the original approach proposed bootstrapping from, exists for only 2 of the 10 archive seasons and would not have worked regardless.

So the approach splits in two:

- **Cross-season FPL identity** (needed for facts, phase 6): a straight join on `code`. `crosswalk/players_fpl.csv` records `(code, canonical_name, name_variants, seasons_seen)`, built mechanically from every season's `players_raw.csv`, with `canonical_name` taken as the most recent spelling. Validation hard-fails on any `code` that maps to two implausibly different names (a genuine collision) and on any player who recorded minutes with no resolvable `code`.
- **Cross-source identity** (FPL ↔ Understat ↔ football-data, needed from phase 7): this is where fuzzy matching and a hand-reviewed CSV remain necessary, since these are genuinely different identifier spaces with no shared stable key. Ingestion hard-fails on any player who recorded minutes and has no mapping and no explicit `no_match` marker.

Team mapping is a small hand-maintained file — there are only 20 teams — extended for the seasons (2016/17–2018/19) that predate vaastav's own `teams.csv`.
...
|---|---|---|
| `daily-snapshot` | 03:30 UTC daily | Runs after FPL's nightly price run. Pulls `bootstrap-static`, `fixtures` and our own `entry` — three requests. Captures prices, ownership, injury news, status, and our bank, squad value and overall rank. Then stages what it pulled, in a second commit: `price_snapshots` and `availability_snapshots` are built by concatenating every `as_of` partition, so raw that is never staged is a series that never grows. Raw is committed and pushed *before* staging is attempted, so a staging failure can never cost a day of the unrecoverable artefact. |
...
| Test | What it proves |
|---|---|
| **Points reconciliation** | Derived points equal FPL's `total_points` for every completed player-fixture, **for all ten archive seasons (2016/17–2025/26), each scored under its own era's ruleset**, zero tolerance. Any mismatch is a rules bug or a data bug. This is the only real proof that scoring — including defensive-contribution thresholds and the 2026/27 BPS changes — is understood correctly. Scoped to 2025/26 only at first (phase 5, the earliest point a reconciliation target exists), extended to all ten seasons once the full backfill lands (phase 6). |
...
## 18. Phase 7 design (recorded 2026-08-02)

Phase 7 stages the four Tier 2 sources named in §6 and assembles one new facts table,
`facts/team_fixture`. It resolves the one open item §15/phase-7 left unpinned — which free
...
One module per source, each following the existing `HttpFetcher`/`Connector` shape:
...
One staging module per source, using the existing `TableSpec`/`stage_frame` framework already
proven by `staging/vaastav.py` and `staging/fpl_api.py` — declared columns and types, an unknown
incoming column is a warning, a missing required one is a failure.

### 18.4 Identity: two crosswalks, two different mechanisms

**Team crosswalk** (`crosswalk/teams.csv`, extending the existing file). Only ~25–30 distinct
clubs span ten seasons across four sources — small enough that a fuzzy-matching pipeline like the
player crosswalk's is unnecessary machinery. Instead: generate a first-draft mapping via simple
name-token matching (the same style of check `identity/players.py::_shares_a_name_token` already
uses), then a human reviews and commits the final CSV once. The committed CSV remains the source
of truth going forward, exactly as `identity/players.py`'s docstring already establishes for the
cross-source case.

**Player crosswalk** (FPL ↔ Understat, `crosswalk/players_fpl_understat.csv`). This is where
genuine fuzzy matching is warranted — thousands of players, no shared stable key across sources.
Reuses the pattern the original spec already locked in at §6/§14: fuzzy-match candidates, a
hand-reviewed CSV as source of truth, and a **hard fail** on any player who recorded FPL minutes
but has no mapping and no explicit `no_match` marker.

### 18.5 `facts/team_fixture` — the new facts table

Grain: **`(season, fixture_id, team_id)`**, mirroring `facts/player_fixture`'s key discipline.
Assembled from the four staged tables joined through the team crosswalk and FPL's own `fixtures`
table. Columns:
...
This table is **silver, not gold**: it describes what was true of the team and fixture, with no
rolling windows, no as-of point-in-time construction, and no modelling. That work belongs to
phase 8's feature library, which will read `facts/team_fixture` the same way it reads
`facts/player_fixture` — keeping phase 8 a pure function of facts, never a place that reconciles
cross-source team identity itself.

### 18.6 Testing

Recorded-response tests per connector (mirroring `tests/sources/test_fpl_api.py` and
`test_vaastav.py`), staging tests per table (mirroring `tests/staging/`), and a facts-assembly
test for `team_fixture` (mirroring `tests/facts/test_player_fixture.py`) — golden cases plus a key
uniqueness check, the same discipline §6 already requires of `player_fixture`.
